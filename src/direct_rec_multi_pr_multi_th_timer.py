...
#
# ========================================
# Bell & Howell Auto 8 Model 172A
# controler program ver.0.0.0
# on Raspberry Pi Zero2W
# image sensor:IMX708
# ========================================
# imwriteをマルチプロセス化 ムービー完了判定タイマーをmulti thread化
...
number_cut          = 0
number_frame        = 0
raw_size            = 0     #0:IMX219 cine, 1:IMX708 cine 2:IMX219 still 3:IMX708 still
raw_pix             = [[1640, 1232], [2304, 1296], [3280, 2464], [4608, 2592], [320, 240]]
rec_size            = 0     #0: cine normal 1: cine L     2:IMX219 full  3:IMX708 full  4:high speed cine
rec_pix             = [[ 640,  480], [ 800,  600], [3208, 2464], [4608, 2592], [320, 240]]
time_log            = []
time_log2           = []
time_log3           = []
time_log4           = []
...
shutter_delay_time  = 10 * 0.001   # シャッター動作を検出してから画像取得するまでの遅延時間 ms

#threads             = []
idle_timer          = None  # ムービー撮影判定用timer    
...
rec_finish_threshold_time
# 最後にシャッターが押された時刻を更新する関数
def update_last_shot_time():
    global idle_timer
    # 既存のタイマーがあればキャンセル
    if idle_timer is not None:
        idle_timer.cancel()
    # 新しいタイマーを設定
    idle_timer = threading.Timer(rec_finish_threshold_time, movie_save)
    idle_timer.start()
...
def movie_save():
    ...
    global number_frame, number_cut, recording_completed, time_log, time_log2, time_log3, time_log4
